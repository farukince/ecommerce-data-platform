...
def insert_sellers(count: int = 30) -> None:
    engine = get_engine()
    rows = generate_sellers(count)

    query = """
    INSERT INTO sellers (seller_name, seller_city, seller_score, is_active)
    VALUES (:seller_name, :seller_city, :seller_score, :is_active)
    """

    execute_many(engine, query, rows)
    print(f"Inserted sellers: {len(rows)}")
...
def insert_products(count: int = 200) -> None:
    ...
    seller_ids = fetch_ids(engine, "sellers", "seller_id")
    rows = generate_products(category_ids, seller_ids, count)

    query = """
    INSERT INTO products (
        seller_id, category_id, product_name, brand, price, product_status
    )
    VALUES (
        :seller_id, :category_id, :product_name, :brand, :price, :product_status
    )
...
def insert_product_inventory() -> None:
    engine = get_engine()
    product_ids = fetch_ids(engine, "products", "product_id")
    rows = generate_product_inventory(product_ids)

    query = """
    INSERT INTO product_inventory (
        product_id, stock_quantity, reserved_quantity, warehouse_location
    )
    VALUES (
        :product_id, :stock_quantity, :reserved_quantity, :warehouse_location
    )
    ...
    execute_many(engine, query, rows)
    print(f"Inserted product_inventory: {len(rows)}")
...
def insert_orders(count: int = 300) -> None:
    ...
    query = """
    INSERT INTO orders (
        user_id, order_status, order_date, total_amount, created_at, updated_at
    )
    VALUES (
        :user_id, :order_status, :order_date, :total_amount, :created_at, :updated_at
    )
...
def insert_payments(count: int = 300) -> None:
    ...
    INSERT INTO payments (
        order_id, payment_status, payment_method, payment_amount, paid_at
    )
    VALUES (
        :order_id, :payment_status, :payment_method, :payment_amount, :paid_at
    )
...
def insert_returns(count: int = 50) -> None:
    ...
    query = """
    INSERT INTO returns (
        order_id, return_reason, return_status, returned_at
    )
    VALUES (
        :order_id, :return_reason, :return_status, :returned_at
    )
